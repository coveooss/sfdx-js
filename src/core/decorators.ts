...
export function apiParameter(apiName: string) {
  return function validate(target: any, propertyName: string, index: number) {
    const calculatedKey =
      propertyName + ":" + DecoratorUtil.getParameterNames(target[propertyName])[index];
    Reflect.defineMetadata(apiParameterMetadataKey, apiName, target, calculatedKey);
  };
}
...
export class DecoratorUtil {
  public static getApiNamespace(resquestClass: Object): string {
    return Reflect.getMetadata(apiNamespaceMetadataKey, resquestClass.constructor);
  }

  public static getApiCommandClass(resquestClass: Object): string {
    return Reflect.getMetadata(apiCommandClassMetadataKey, resquestClass.constructor);
  }

  public static getApiCommand(requestObject: Object, functionName: string): string {
    return Reflect.getMetadata(apiCommandMetadataKey, requestObject, functionName);
  }

  public static getApiParameter(parameterName: string, methodName: string, requestObject: Object) {
    let calculatedKey = methodName + ":" + parameterName;
    return Reflect.getMetadata(apiParameterMetadataKey, requestObject, calculatedKey);
  }
